...
int main(int argc, char **argv) {
  int dispwidth = 640;
  int dispheight = 480;
  int dispres = 72; // check? leave as default?
  int dispdepth = gr_pixel_mode_rgb24; // or _gray
  double size = -1; // wut
  ...
  grBitmap scrbuf;
  scrbuf.mode = dispdepth;
  scrbuf.width = dispwidth;
  scrbuf.rows = dispheight;
  scrbuf.grays = 256;
 
  grSurface *surface = grNewSurface(0, &scrbuf);
  if (!surface) { printf("NEWSURFACEFAIL\n"); break; }

  grBitmap *surfacebmp = &surface->bitmap;
  printf("scrbuf&: %12lx, surfacebmp: %12lx\n", (unsigned long)(void *)&scrbuf, (unsigned long)(void *)surfacebmp);

//  if (!surface) { printf("SURFACEFAIL\n"); return 2; }
//  else {
  printf("default GAMMA: %f\n", GAMMA);
    // grSetTargetGamma(bsurface, GAMMA);
  UNT ttinterpreterversion;
  FT_Property_Get(ftlib, "truetype", "interpreter-version", &ttinterpreterversion);
  printf("TrueType interpreter v%d\n", ttinterpreterversion);
...
//    { render_state_set_resolution(state, (UNT)dispres); }
  ...
  //UNT defaultttinterpreterversion;
  //FT_Property_Get(handle->library, "truetype", "interpreter-version", &status.ttinterpreterversion);
  //int i = -1;
  //while (++i < 3)
...
*/
  ...
  ELSEDO
#define PIXELWIDTH  700
#define PIXELHEIGHT 500
// actual srcbmp may be a different size !
// each actual glyph may be a different size !

  error = FT_Set_Pixel_Sizes(fontface, PIXELWIDTH, PIXELHEIGHT); // WIDTH AND HEIGHT !!!!!
...
//    grFillRect(display->bitmap, 0, 0, 640, 480, white); //display->warn_color);
    grFillRect(surfacebmp, 0, 0, 640, 420, white); //display->warn_color);
    error = rendertext(surfacebmp, &fontface, Text); // add properties parameter
    if (error) { printf("RENDERTEXT ERROR\n"); return 3; } // <- unsafe exit.. break?
    //grRefreshSurface(display->surface);
    grRefreshSurface(surface);
    grListenSurface(surface, 0, &grevent);
